...
- **Minimize Boilerplate Penalties**: Don't penalize missing imports, comment blocks, or scaffolding code unless they directly affect functionality or are explicitly required by the assignment.
...
- **Point, Don't Solve**: Frame suggestions around the pattern or place to re-check, not style dogma (e.g., "look at the loop bounds in your second cell" for an off-by-one error). Never write the corrected code — leave the last step to the student.
