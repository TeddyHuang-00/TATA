...
> v4 (2026-09-01 update)：Tab 级 4 个（Dashboard/Plagiarism/**Library**/Settings，v3 已加 Library）；**Library tab = 三个 pane**：Rubrics / Prompts / **Providers**（v4 新增，`config/provider.toml` 注册表编辑 + Test connection，见 §10）
...
| S5 | Settings | provider（只读注册表）/ Canvas（.env 编辑）/ plagiarism 权重 / 路径 配置编辑（上下文选择器：Global/Course/Assignment 三级） | `schema`（生成路径说明） |
| S6 | Library（v3 新增） | 三个 pane：**Rubrics**（builder + 列表）、**Prompts**（TextArea 编辑 `data/prompt/*.md`）、**Providers**（v4：`config/provider.toml` 注册表 添加/删除/编辑 + Test connection；Settings 的 provider 列表保持只读） | —（TUI 内编辑） |
...
> S1 的 Global/Course/Assignment 是**同一 Tab 内的视图切换**（`state.dashboard_level`），非 Tab 切换；Tab 级 4 个（Dashboard/Plagiarism/Library/Settings——v3 起，Library 位于 Plagiarism 与 Settings 之间）。
...
**结论：** 平台外壳（Header/Footer/全局绑定 `q` `tab` `?`）常驻；4 个工作区用 `TabbedContent`（Dashboard/Plagiarism/Library/Settings）；S1 内部三层用**视图栈 drill-down**（非 Tab，见 01 §1）；Review 与全部弹窗用 `push_screen`/`pop_screen`。
...
- 编辑入口：**provider 注册表编辑已从 Settings 迁至此处**（Settings 只保留 `#grading-registry` 只读展示）；rubric/prompt 编辑同理。
