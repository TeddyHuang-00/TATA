...
> 依据源码：`main.py`（`_STAGES` 注册表、各 stage 纯函数）、`src/score_review.py`（既有 Viewer App）、`src/assignment_config.py`（config 分层模型）、`assignments/config.toml`（迁移后=global 根配置样例）
...
| `courses` | list[CourseInfo] | 扫描 `assignments/*/` 中 course 目录（含 `config.toml` 且子目录=作业） | Global 视图 |
...
```
dir_name: str            # assignments/<dir> 目录名（= 课程 slug）
config_path: Path        # assignments/<dir>/config.toml
course_id: int|None      # merged [fetch].course_id
assignment_count: int
counts: {raw, processed, graded, scores}   # Σ 旗下作业
plagiarism_flags: int    # Σ 各作业 pair $error 判定数（聚合 z 超 alpha）
...
```
① 导入课程  S1·Global: c → Modal(Canvas 选课程) → 建 assignments/<slug>/config.toml → 进入 Course 视图
...
| `assignments/` 不存在 | Global 空态：提示 + 「从 Canvas 导入第一个课程」按钮（仍可用，导入后自动生成目录） |
