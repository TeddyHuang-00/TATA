...
> v4 (2026-09-01 update)：Tab 级 3 个（Dashboard/**Library**/Settings——S4 Plagiarism 已嵌入 Dashboard 的 course 视图，v3 已加 Library）；**Library tab = 三个 pane**：Rubrics / Prompts / **Providers**（v4 新增，`config/provider.toml` 注册表编辑 + Test connection，见 §10）
...
| #   | 屏幕                                | 职责                                                                                                                                                                                                                  | 对应 CLI 子命令                                                    |
| --- | ----------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------ |
| S1  | Dashboard（三层）                   | **Global**：课程一览 + 导入课程 + 全局配置 ｜ **Course**：课程作业一览 + 导入作业/fetch 全部/查重+聚合/课程配置 ｜ **Assignment**：单作业工作台（原 S2，含 pipeline 全 stage）                                        | `fetch`、`plagiarism --aggregate`、`list_courses/list_assignments` |
...
| S5  | Settings                            | provider（Select，来源 config/provider.toml）/ Canvas（.env 编辑）/ plagiarism 权重 / 路径 配置编辑（上下文选择器：Global/Course/Assignment 三级）                                                                    | `schema`（生成路径说明）                                           |
| S6  | Library（v3 新增）                  | 三个 pane：**Rubrics**（builder + 列表）、**Prompts**（TextArea 编辑 `data/prompt/*.md`）、**Providers**（v4：`config/provider.toml` 注册表 添加/删除/编辑 + Test connection；Settings 的 provider 下拉不展示注册表） | —（TUI 内编辑）                                                    |
...
```mermaid
flowchart TD
    A[TataApp<br/>Header + 3 Tabs + Footer] --> D1[S1 Dashboard]
    A --> D6[S6 Library<br/>Rubrics / Prompts / Providers]
    A --> D5[S5 Settings]
    subgraph S1 三层 drill-down
        ...
        C -- c 导入作业 / F fetch全部 --> M1{确认 Modal + job}
        C -- p 查重+聚合 --> M2{确认 Modal + job}
        C -- o 课程配置 / G g 全局配置 --> D5
        G -- g 全局配置 --> D5
    end
    C -- s 键 / Review 按钮 --> D3[S3 Review<br/>push_screen]
    D3 -- esc 返回 --> C
    C -. S4 嵌入 course 视图（作业表下方） .-> P[S4 Plagiarism<br/>4-tab 排名表]
    P -- 行选中 --> M4{push 详情 Screen<br/>Aggregate/Assignment/Student/Pair}
    P -. Pairs 行高亮 .-> CMP[#cmp-pane 内嵌对比]
    D5 -- 保存校验失败 --> M3{Modal: 错误详情}
```

> S1 的 Global/Course/Assignment 是**同一 Tab 内的视图切换**（`state.dashboard_level`），非 Tab 切换；Tab 级 3 个（Dashboard/Library/Settings——S4 Plagiarism 嵌入 Dashboard course 视图）。
...
**结论：** 平台外壳（Header/Footer/全局绑定 `q` `tab` `?`）常驻；3 个工作区用 `TabbedContent`（Dashboard/Library/Settings——S4 Plagiarism 嵌入 Dashboard course 视图）；S1 内部三层用**视图栈 drill-down**（非 Tab，见 01 §1）；Review 与全部弹窗用 `push_screen`/`pop_screen`。
...
| 键    | 全局动作              | 备注                                                       |
| ----- | --------------------- | ---------------------------------------------------------- |
| `q`   | 退出                  | 立即退出、无确认（无 job 确认；known deviation，同 01 §5） |
| `?`   | 打开帮助面板          | 应用级 `toggle_help`（Keys 面板开关；`f1` 未绑定）         |
...
② 导入作业  S1·Course: c → Modal(作业点选) → fetch 后台跑 → 写 course config 清单（`id` 条目）
...
## 10. Library tab（v3 新增，v4 补 Providers pane）

`LibraryScreen` 是非 push 的 tab 内组件，`TabbedContent` 三个 pane：

| pane                        | 内容                                                                                                                                                                                                                                                                                                                           |
| --------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Rubrics                     | `RubricsPane`（原 `RubricBuilderScreen` 迁入，不再 push_screen）：builder 表单 + 列表；删除前确认显示引用 config 数；改名联动更新引用 config 的 `[grading]`                                                                                                                                                                    |
| Prompts                     | `PromptsPane`：TextArea 编辑 `data/prompt/*.md`，新建/重命名/删除 + 引用确认                                                                                                                                                                                                                                                   |
| Providers（v4，2026-09-01） | `ProvidersPane`：`config/provider.toml` 注册表 添加/删除/编辑（base_url/api_key/model/mode/temperature），tomlkit 字段级编辑、注释保留；**Test connection**（worker 线程 OpenAI ping，`max_tokens=1`）；删除前确认并显示引用它的 `[grading].provider` config 数；`config_path` 可注入（默认 `REPO_ROOT/config/provider.toml`） |

- 编辑入口：**provider 注册表编辑已从 Settings 迁至此处**（Settings 的 provider 字段仍读自 `config/provider.toml`）；rubric/prompt 编辑同理。
- 已知边界：Providers 只支持增删改，不支持重命名（改名 = 删除 + 新建，引用关系需手动更新，或参考 Prompts/Rubrics 的联动改名——未实现）。
- 组件均为非 Screen widget → 样式走 `DEFAULT_CSS`（`src/tui/styles/library.tcss`，教训 c9272e81）。
