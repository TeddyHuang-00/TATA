...
> 对应 CLI：`fetch / preprocess / grade / score / analyze`（查重不在作业工作台：Course 层 `p` 与 S4，见 04）；长任务后台线程化
> 关键源码事实：`_STAGE_KEYS` 注册 `fetch / preprocess / grade / score / analyze / score review` 六段（`score review` = `open_score_review` → push_screen(ScoreReviewScreen)）；`GradingCheckpoint.done` 清单；fetch 有 `.fetch-cache.json`；preprocess 可做 mtime 增量
...
│ └────────────┘  └────────────┘ │  [e]Edit config  [settings]Job settings    │
│ ┌────────────┐  ┌────────────┐ │              (v2)                          │
│ │A analyze   │  │R score rev.│ │                                            │
│ │stats done  │  │(open)      │ │                                            │
│ └────────────┘  └────────────┘ │                                            │
├────────────────────────────────┴─────────────────────────────────────────────┤
...
| Stage 按钮 | 6× `Button`（`Vertical` 内两列 `Grid`，class `stage-btn`） | F/P/G/S/A + score review 六段（查重已移除）；副标题行显示增量计数（Button 内 `\n` 两行文本） |
...
> v1.1 (2026-08-30 update)：查重不在作业工作台 —— 移除 `k`/`;` 与「Run plagiarism」按钮；查重仅剩 Course 层 `p`（全作业检测 + 聚合）与 S4 查看（04）；新增 `score review` stage 按钮（点击 push_screen ScoreReviewScreen，无键位）
...
```
▶ To run: fetch 0 · pre 0 · grade 8 · score 0  |  Skip: 21  |  No change: 3
```
...
| **Empty·no scores** | `score review` stage 按钮可点：`open_score_review` → push_screen(ScoreReviewScreen)，无评分时 reviewer 显示空表 |
