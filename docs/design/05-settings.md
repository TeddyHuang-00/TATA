...
| 上下文         | 可编辑内容                                                                                                                                                                                                     | 写入目标                           |
| -------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------- |
| **Global**     | 环境 `.env`（`CANVAS_BASE_URL`/`CANVAS_ACCESS_TOKEN`，Canvas tab 编辑 + Save .env）；global config 可选的跨课程默认（[plagiarism] 权重/阈值）。（v4：provider 注册表编辑迁至 **Library tab → ProvidersPane**） | `.env` / `data/config.toml`        |
